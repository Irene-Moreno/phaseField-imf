...
void variableAttributeLoader::loadVariableAttributes(){
    ...
    set_dependencies_gradient_term_RHS(0, "c,grad(mu)");
...
scalargradType mux = variable_list.get_scalar_gradient(1);  // mux = grad(betamu)

// --- Setting the expressions for the terms in the governing equations ---
scalarvalueType eq_c = c;
scalargradType eqx_c = constV(-DcV*cTcV*userInputs.dtValue)*c*mux;
...
void customPDE<dim,degree>::nonExplicitEquationRHS(variableContainer<dim,degree,dealii::VectorizedArray<double> > & variable_list,
 ...
 scalarvalueType fcV = std::log(c) + AcV*(1.0-c)*(1.0-c);

 // The terms for the governing equations
 scalarvalueType eq_mu = fcV;
 scalargradType eqx_mu = constV(KcV*cTcV)*cx;
